...
/**
 * @fileOverview A C++ code snippet generation AI agent.
 *
 * - generateCodeSnippet - A function that handles the C++ code snippet generation process.
...
const GenerateCodeSnippetInputSchema = z.object({
  description: z.string().describe('The description of the desired C++ code snippet.'),
});
...
const GenerateCodeSnippetOutputSchema = z.object({
  codeSnippet: z.string().describe('The generated C++ code snippet.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an AI expert in C++ code generation.

You will generate a C++ code snippet based on the user's description.

Description: {{{description}}}

Ensure the code is well-formatted and includes necessary comments.
`,
});
